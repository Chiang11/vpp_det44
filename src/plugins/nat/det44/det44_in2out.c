  ...
  while (n_left_from > 0) // 最后剩下一个，就处理一个
    {
      ...
      kv.key = ip0->src_address.as_u32 & ip4_main.fib_masks[MY_PLEN];
      kv.value = 0;
      value.key = 0;
      value.value = 0;
      clib_bihash_search_8_8 (&det44_main.hash_table, &kv, &value);
      ...
      my_session_table_t *table0 = &mp0->my_sessions_tables[addr_offset];
      ...
      for (i0 = 0; i0 < MY_SESSIONS_PER_EXTERNAL_ADDR / 2; i0++)
        {
          my_session_t *ses0 = &table0->my_sessions[i0 + mapped_port];
          if (ses0->expire <= now)
            {
              continue; // 跳过已超时的会话
            }
          if (ses0->in_port == udp0->src_port &&
      ...
      if (PREDICT_FALSE (!ses0))
        {
          ...
          for (i0 = 0; i0 < MY_SESSIONS_PER_EXTERNAL_ADDR / 2; i0++)
            {
              my_session_t *s0 = &table0->my_sessions[i0 + mapped_port];
              ...
              if (s0->in_port == 0)
                {
                  if (clib_atomic_bool_cmp_and_swap (&s0->in_port, 0,
                                                     udp0->src_port))
                    {
                      ...
                      s0->expire = now + dm->timeouts.tcp.transitory;
      ...
      vlib_cli_output (vm, "sessions tables number: %d, in %U/%d out %U/%d\n", value.value, format_ip4_address,
          &ip0->src_address, udp0->src_port, format_ip4_address, &mapped_addr, mapped_port);
