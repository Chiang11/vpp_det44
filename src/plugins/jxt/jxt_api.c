...
#include <plugins/jxt/jxt.api_enum.h>
#include <plugins/jxt/jxt.api_types.h>
...
static void
vl_api_jxt_add_del_map_t_handler (vl_api_jxt_add_del_map_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_add_del_map_reply_t *rmp;
  int rv = 0;
  ...
  REPLY_MACRO (VL_API_jxt_ADD_DEL_MAP_REPLY);
}

static void
vl_api_jxt_forward_t_handler (vl_api_jxt_forward_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_forward_reply_t *rmp;
  int rv = 0;
...
send_reply:
  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_jxt_FORWARD_REPLY,
  ({
...
static void
vl_api_jxt_reverse_t_handler (vl_api_jxt_reverse_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_reverse_reply_t *rmp;
  int rv = 0;
...
send_reply:
  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_jxt_REVERSE_REPLY,
  ({
...
static void
sent_jxt_map_details (snat_det_map_t * m, vl_api_registration_t * reg,
			u32 context)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_map_details_t *rmp;

  rmp = vl_msg_api_alloc (sizeof (*rmp));
  clib_memset (rmp, 0, sizeof (*rmp));
  rmp->_vl_msg_id = ntohs (VL_API_jxt_MAP_DETAILS + dm->msg_id_base);
...
static void
vl_api_jxt_map_dump_t_handler (vl_api_jxt_map_dump_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  ...
  vec_foreach(m, dm->det_maps)
    sent_jxt_map_details(m, reg, mp->context);
  /* *INDENT-ON* */
}

static void
vl_api_jxt_close_session_out_t_handler (vl_api_jxt_close_session_out_t
					  * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_close_session_out_reply_t *rmp;
...
send_reply:
  REPLY_MACRO (VL_API_jxt_CLOSE_SESSION_OUT_REPLY);
}

static void
vl_api_jxt_close_session_in_t_handler (vl_api_jxt_close_session_in_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_close_session_in_reply_t *rmp;
...
static void
send_jxt_session_details (snat_det_session_t * s,
			    vl_api_registration_t * reg, u32 context)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_session_details_t *rmp;

  rmp = vl_msg_api_alloc (sizeof (*rmp));
  clib_memset (rmp, 0, sizeof (*rmp));
  rmp->_vl_msg_id = ntohs (VL_API_jxt_SESSION_DETAILS + dm->msg_id_base);
...
static void
vl_api_jxt_session_dump_t_handler (vl_api_jxt_session_dump_t * mp)
{
  vl_api_registration_t *reg;
  ...
  for (i = 0; i < jxt_SES_PER_USER; i++)
    {
      if (s->out.as_u64)
	send_jxt_session_details (s, reg, mp->context);
      s++;
    }
}

static void
  vl_api_jxt_plugin_enable_disable_t_handler
  (vl_api_jxt_plugin_enable_disable_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_plugin_enable_disable_reply_t *rmp;
  jxt_config_t c = { 0 };
  int rv = 0;
  if (mp->enable)
    {
      ...
      rv = jxt_plugin_enable (c);
    }
  else
    {
      rv = jxt_plugin_disable ();
    }
  REPLY_MACRO (VL_API_jxt_PLUGIN_ENABLE_DISABLE_REPLY);
}

static void
  vl_api_jxt_interface_add_del_feature_t_handler
  (vl_api_jxt_interface_add_del_feature_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_interface_add_del_feature_reply_t *rmp;
  u32 sw_if_index = ntohl (mp->sw_if_index);
  int rv = 0;
  VALIDATE_SW_IF_INDEX (mp);
  rv = jxt_interface_add_del (sw_if_index, mp->is_inside, !mp->is_add);
  BAD_SW_IF_INDEX_LABEL;
  REPLY_MACRO (VL_API_jxt_INTERFACE_ADD_DEL_FEATURE_REPLY);
}

static void
jxt_send_interface_details (jxt_interface_t * i,
			      vl_api_registration_t * reg, u32 context)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_interface_details_t *rmp;

  rmp = vl_msg_api_alloc (sizeof (*rmp));
  clib_memset (rmp, 0, sizeof (*rmp));
  rmp->_vl_msg_id = ntohs (VL_API_jxt_INTERFACE_DETAILS + dm->msg_id_base);
  rmp->sw_if_index = ntohl (i->sw_if_index);
  rmp->is_outside = jxt_interface_is_outside (i);
  rmp->is_inside = jxt_interface_is_inside (i);
  rmp->context = context;
  vl_api_send_msg (reg, (u8 *) rmp);
}

static void
vl_api_jxt_interface_dump_t_handler (vl_api_jxt_interface_dump_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_registration_t *reg;
  jxt_interface_t *i;
  ...
  pool_foreach (i, dm->interfaces)
   {
    jxt_send_interface_details(i, reg, mp->context);
  }
  /* *INDENT-ON* */
}

static void
vl_api_jxt_set_timeouts_t_handler (vl_api_jxt_set_timeouts_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_set_timeouts_reply_t *rmp;
  ...
  rv = jxt_set_timeouts (&timeouts);
  REPLY_MACRO (VL_API_jxt_SET_TIMEOUTS_REPLY);
}

static void
vl_api_jxt_get_timeouts_t_handler (vl_api_jxt_get_timeouts_t * mp)
{
  jxt_main_t *dm = &jxt_main;
  vl_api_jxt_get_timeouts_reply_t *rmp;
  nat_timeouts_t timeouts;
  int rv = 0;
  timeouts = jxt_get_timeouts ();
  /* *INDENT-OFF* */
  REPLY_MACRO2 (VL_API_jxt_GET_TIMEOUTS_REPLY,
  ({
...
#include <plugins/jxt/jxt.api.c>

/* Set up the API message handling tables */
clib_error_t *
jxt_api_hookup (vlib_main_t * vm)
{
  jxt_main_t *dm = &jxt_main;
